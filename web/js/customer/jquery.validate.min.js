...
!function (a) {
    ...
    }, messages: {required: "必填字段", remote: "请修改此处", email: "请输入正确格式的电子邮箱", url: "请输入正确格式的网址", date: "请输入正确格式的日期.", dateISO: "请输入正确格式的日期(ISO)", number: "必须输入合法的数字(负数，小数)", digits: "必须输入整数", creditcard: "必须输入合法的信用卡号", equalTo: "与上一次输入不一致", maxlength: a.validator.format("请输入不超过0个字"), minlength: a.validator.format("请输入至少0个字"), rangelength: a.validator.format(" 输入长度必须介于 5 和 10 之间的字"), range: a.validator.format("输入值必须介于 5 和 10 之间."), max: a.validator.format("输入值不能大于10"), min: a.validator.format("输入值不能小于1")}, autoCreateRanges: !1, prototype: {init: function () {
        function b(b) {
